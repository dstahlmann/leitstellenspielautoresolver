...
function bootstrap()
{
	...
	if( $( 'img' ).attr('vehicle_type_id') == 28 || $( 'img' ).attr('vehicle_type_id') == 31 )
	{ 		
		collectHospitals();
		sortByDistance();
...
{
	buttons = $( '.btn-success' );
	nextPrison = buttons[1].innerHTML;
	nextPrison.substr( nextPrison.lastIndexOf( ':' )+2, nextPrison.lastIndexOf( ',' )-nextPrison.lastIndexOf( ':' )-2 );
	...
	$( "a[href='"+ targetURL +"/gefangene/entlassen']" ).click();	
	}
